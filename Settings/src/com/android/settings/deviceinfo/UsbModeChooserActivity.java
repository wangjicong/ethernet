...
// add by maohaojie for bug 19333 on 2018.09.21 
...
public class UsbModeChooserActivity extends Activity {
	...
	// add by maohaojie for bug 19333 on 2018.09.21
	private static final String USB_MODE_ENABLE = "persist.sys.usbmode";
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
	    ...
	    // add by maohaojie for bug 19333 on 2018.09.21
	    String value = SystemProperties.get(USB_MODE_ENABLE);
        ...
        // add by maohaojie for bug 19333 on 2018.11.22 start 
        if(value.equals("true")){
            SystemProperties.set(USB_MODE_ENABLE, "false");
            //modify by wangxing for bug P_RK95_E-378 start       
            // mDialog.show();
            finish();
            return;
            //modify by wangxing for bug P_RK95_E-378 end
        }else{
            mDialog.show();
        }
        // add by maohaojie for bug 19333 on 2018.11.22 end		
        ...
        for (int i = 0; i < DEFAULT_MODES.length; i++) {
            if (mBackend.isModeSupported(DEFAULT_MODES[i])
                    && !mBackend.isModeDisallowedBySystem(DEFAULT_MODES[i])) {
                    	boolean isEq = (current == DEFAULT_MODES[i]);
    ...
    private void inflateOption(final int mode, boolean selected, LinearLayout container,
        ...
        v.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                finish();
                // add by maohaojie for bug 19333 on 2018.09.21
                if(!selected){//modify by wangxing for bug P_RK95_E-378 
                    SystemProperties.set(USB_MODE_ENABLE, "true");
                }
            }
        });
    ...
    static void updateSummary(TextView summaryView, int mode) {
        ...
        } else if (mode == (UsbBackend.MODE_POWER_SINK | UsbBackend.MODE_DATA_NONE)){
            summaryView.setText(R.string.usb_use_charging_only_desc);
        } else if (mode == (UsbBackend.MODE_POWER_SINK | UsbBackend.MODE_DATA_MTP )){
            summaryView.setText(R.string.usb_use_file_transfers_desc);
        } else if (mode == (UsbBackend.MODE_POWER_SINK | UsbBackend.MODE_DATA_PTP )){
            summaryView.setText(R.string.usb_use_photo_transfers_desc);
        } else if (mode == (UsbBackend.MODE_POWER_SINK | UsbBackend.MODE_DATA_MIDI )){
